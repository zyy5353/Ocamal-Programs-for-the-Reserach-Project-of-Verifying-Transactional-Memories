(* reference automaton with non-tx instructions which satisfies strict serializability *)
...
let commit_t1 ((sta1, flg1, non_tx1, rs1, ws1, prs1, pws1, preds1): state) ((sta2, flg2, non_tx2, rs2, ws2, prs2, pws2, preds2): state) =  begin
  if (non_tx1 != 0) || (non_tx2 != 0) then ((Non_exist, 0, 0, (None, None), (None, None), (None, None), (None, None), None), (Non_exist, 0, 0, (None, None), (None, None), (None, None), (None, None), None))
    ...
    if sta1 = Finished || sta1 = Serialized then begin
      ...
      let preds = None in
      if sta2 = Finished then ((sta, flg1, non_tx1, rs, ws, prs, pws, preds), (sta2, flg2, non_tx2, rs2, ws2, prs2, pws2, preds2))
      else begin
	match preds1 with 
	| Some T2 -> begin
...
let commit_t2 ((sta1, flg1, non_tx1, rs1, ws1, prs1, pws1, preds1): state) ((sta2, flg2, non_tx2, rs2, ws2, prs2, pws2, preds2): state) =  begin
  if (non_tx1 != 0) || (non_tx2 != 0) then ((Non_exist, 0, 0, (None, None), (None, None), (None, None), (None, None), None), (Non_exist, 0, 0, (None, None), (None, None), (None, None), (None, None), None))
    ...
    if sta2 = Finished || sta2 = Serialized then begin
      ...
      let pws = (None, None) in
      let preds = None in
      if sta1 = Finished then ((sta1, flg1, non_tx1, rs1, ws1, prs1, pws1, preds1), (sta, flg2, non_tx2, rs, ws, prs, pws, preds)) 
      else begin
	match preds2 with
	| Some T1 -> begin
...
let  get_post_state state1 state2 = begin
  ...
  (* let rec inv_remove inval_list valid_list = *)
  (*   begin  *)
  (*     match inval_list, valid_list with *)
  (*     |	[], v -> v *)
  (*     |	hd::tl, v -> begin *)
  (* 	  match (check_status hd 1), (check_status hd 2) with *)
  (* 	  | Invalid, _ ->  *)
  (* 	      begin  *)
  (* 		print_states_to_file hd;  *)
  (* 		state_no := (!state_no) + 1; *)
  (* 		inv_remove tl v  *)
  (* 	      end *)
  (* 	  | _, Invalid -> *)
  ...
  (* 		inv_remove tl v  *)
  (* 	      end *)
  (* 	  | _, _ -> inv_remove tl (hd::v) *)
  (*     end *)
  (*   end *)
  (* in *)
  
  (* let finalList = inv_remove newList [] in *)
...
let w_file = "Non_tx_reference_new_invalid_automaton.ml"
